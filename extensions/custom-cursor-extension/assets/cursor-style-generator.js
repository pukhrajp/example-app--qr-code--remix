/**
 * Cursor Style Generator (Browser Version)
 * 
...
(function(window) {
   ...
   */
  function validateHotspot(value, defaultValue, min, max) {
    // Set defaults
    defaultValue = defaultValue !== undefined ? defaultValue : 0;
    min = min !== undefined ? min : 0;
    max = max !== undefined ? max : 128;
   ...
   */
  function generateCursorCSS(cursorData) {
    ...
    const imageUrl = cursor.imageUrl;
    const hoverImageUrl = cursor.hoverImageUrl;
    const hotspotX = cursor.hotspotX;
    const hotspotY = cursor.hotspotY;
    const size = cursor.size;

    // Sanitize and validate imageUrl
    const sanitizedImageUrl = sanitizeURL(imageUrl);
    if (!sanitizedImageUrl) {
      console.warn('[Custom Cursor] Invalid or unsafe imageUrl provided:', imageUrl);
      return null;
    }
  ...
  // Export to global namespace for use in cursor-loader.js
  window.CustomCursorStyleGenerator = {
    generateCursorCSS: generateCursorCSS,
    // Expose helper functions for potential external use
    sanitizeURL: sanitizeURL,
    validateHotspot: validateHotspot,

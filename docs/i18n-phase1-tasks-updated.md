# Phase 1: Setup & Infrastructure - Task Breakdown (UPDATED)

## 📋 Overview
Breaking down Phase 1 into small, testable increments for safe implementation.

**Update:** Using `@shopify/i18next-shopify` (Official Shopify package) instead of vanilla i18next
...
- This package includes `i18next` and `react-i18next` as peer dependencies
- Update `package.json`
- Verify installation
...
- ✅ **Built-in App Bridge integration** (auto-detects merchant locale)
- ✅ **Automatic locale detection** from Shopify session
- ✅ **Optimized for Shopify apps** (follows Shopify patterns)
- ✅ **Includes i18next and react-i18next** as dependencies
- ✅ **Type-safe** with TypeScript support
- ✅ **Best practice** recommended by Shopify
...
2. ✅ Check `node_modules` for `@shopify/i18next-shopify`
3. ✅ Verify peer dependencies installed (`i18next`, `react-i18next`)
4. ✅ Run `npm run dev` to ensure no conflicts
5. ✅ Verify app still runs without errors

### Success Criteria:
- No installation errors
- App still runs normally
- `@shopify/i18next-shopify` appears in `package.json`
...
## Task 1.3: Create i18n Configuration File ⏱️ 25 minutes

### What We'll Do:
- Create `app/i18n/config.js`
- Configure i18next using `@shopify/i18next-shopify`
- Set up Shopify App Bridge integration
- Configure English as default language
...
```javascript
import { initI18next } from '@shopify/i18next-shopify';
...
const i18n = initI18next({
  resources: {
    en: {
      common: {}, // Will populate in Task 1.4
      cursors: {},
      errors: {},
    },
  },
...
### Key Differences from vanilla i18next:
- Uses `initI18next` from `@shopify/i18next-shopify`
- Automatically connects to Shopify App Bridge
- Detects merchant's locale from Shopify session
- Follows Shopify's i18n conventions
...
5. ✅ Verify App Bridge integration doesn't throw errors
...
- App still runs normally
- ESLint passes
- App Bridge integration configured

**Status:** Ready to implement  
**Risk Level:** Low-Medium (config file, Shopify integration)
...
## Task 1.5: Create i18n Provider Component ⏱️ 25 minutes

### What We'll Do:
- Create `app/i18n/I18nProvider.jsx` component
- Use `@shopify/i18next-shopify` hooks
- Initialize i18next with Shopify App Bridge
...
### Key Benefits:
- Automatic Shopify App Bridge connection
- Merchant locale detection (from shop settings)
- Seamless integration with Polaris
- Type-safe translation keys (if using TypeScript later)
...
5. ✅ Shopify App Bridge integration working

### Success Criteria:
- Provider component created
- i18next initializes successfully with Shopify
- No console errors
- App still runs

**Status:** Ready to implement  
**Risk Level:** Medium (involves React context & Shopify integration)
...
6. ✅ Shopify App Bridge working correctly
...
- Verify translations work end-to-end with Shopify integration
...
2. ✅ No console errors
3. ✅ Change translation in JSON to verify it updates (hot reload)
4. ✅ Verify fallback works (delete translation, should show key)
5. ✅ All other functionality still works
6. ✅ Shopify App Bridge locale detection working
7. ✅ Verify translation loads from correct namespace

### Success Criteria:
- ONE translation working
- Proof of concept successful with Shopify integration
- No side effects
...
|------|-------------|------|------|--------|
| 1.1 | Install `@shopify/i18next-shopify` | 10 min | Low | Ready |
| 1.2 | Create Directories | 5 min | Zero | Ready |
| 1.3 | Create Config (Shopify integration) | 25 min | Low-Medium | Ready |
| 1.4 | Create Translation Files | 30 min | Low | Ready |
| 1.5 | Create i18n Provider | 25 min | Medium | Ready |
...
- [ ] Shopify App Bridge working (for Tasks 1.3+)
...
1. **Task 1.3** - Review Shopify integration configuration
2. **Task 1.4** - Review translation files (you can edit/improve strings)
3. **Task 1.6** - Review integration (ensure app still works)
4. **Task 1.7** - Review proof of concept (ONE translation working with Shopify)
...
- ✅ `@shopify/i18next-shopify` is set up
- ✅ Shopify App Bridge integration working
- ✅ Automatic locale detection from merchant's shop
...
**Note:** `@shopify/i18next-shopify` includes `i18next` and `react-i18next` as peer dependencies, so they will be automatically installed.

---

## 🎁 Benefits of @shopify/i18next-shopify

1. **App Bridge Integration** - Automatically connects to Shopify App Bridge
2. **Locale Detection** - Detects merchant's locale from shop settings
3. **Polaris Compatibility** - Works seamlessly with Polaris components
4. **Shopify Standards** - Follows Shopify's recommended patterns
5. **Type Safety** - Full TypeScript support (if we need it later)
6. **Maintained** - Officially maintained by Shopify
7. **Future-Proof** - Updates with Shopify platform changes
...
- Shopify integration will automatically handle locale switching
...
1. Does this updated task breakdown look good?
2. Are you comfortable with using `@shopify/i18next-shopify`?
3. Any concerns about Shopify App Bridge integration?
...
**Document Version:** 2.0 (Updated for @shopify/i18next-shopify)  
